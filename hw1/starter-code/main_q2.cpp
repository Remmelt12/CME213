...
std::vector<uint> blockInd(uint numKeys, uint numBlocks)
{
    uint dist =  numKeys/numBlocks;
    std::vector<uint> indices(numBlocks+1,dist);
    indices[0]=0;
    if(dist*numBlocks==numKeys){
        ...
        uint diff= numKeys-dist*numBlocks;
        uint i=1;  
        while(diff!=0)
        {
            indices[i]++;
...
std::vector<uint> computeBlockHistograms(const std::vector<uint>& keys,
    ...
    std::vector<uint> blocksize = blockInd(keys.size(),numBlocks);
    #pragma omp parallel for
    for(uint i = 0; i < numBlocks; ++i){
        uint block = blocksize[i+1]-blocksize[i]; 
        for (uint j=0;j<block; ++j){
            uint key = (keys[blocksize[i]+j] >> startBit) & mask;
...
void populateOutputFromBlockExScan(const std::vector<uint>& blockExScan,
    ...
    for(uint i=0;i<numBlocks;i++)
    {
        ...
        for (uint j=0;j<block; ++j){
            uint key = (keys[blocksize[i]+j] >> startBit) & mask;
            // Find bucket to which key belongs
            uint index = blockExScan[key+i*numBuckets]+local_offset[key];
            sorted[index] = keys[blocksize[i]+j];
            local_offset[key]++;
        }
    }
}
